...
trait ValidationFormTrait
{
	...
	public function validator($rules, $value, $values)
	{
		$errors = [];
	 ...
	 * @return mixed
	 */
	private function validateMax($rule, $value)
	{
		$size = $rule['size'];
		$message = $rule['message'] ? $rule['message'] : "This field must not exceed $size characters";

		if (gettype($value) === 'string' &&  count($value) > $size) return $message;
		if (gettype($value) === 'integer' &&  $value > $size) return $message;
	 ...
	 * @return mixed
	 */
	private function validateMin($rule, $value)
	{
		$size = $rule['size'];
		$message = $rule['message'] ? $rule['message'] : "This field must have at least $size characters";
	 ...
	 * @return mixed
	 */
	private function validateMatch($rule, $value)
	{
		$message = $rule['message'] ? $rule['message'] : "This field is not match";
